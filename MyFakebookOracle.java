    ...
    public void matchMaker(int n, int yearDiff) {
        try (Statement stmt=
            ...
            ResultSet rst=stmt.executeQuery("select distinct U1.user_id, U2.user_id, COUNT(T1.tag_photo_id) from "+userTableName+" U1, "+userTableName+" U2, "
                ...
                AND T1.tag_photo_id=T2.tag_photo_id AND U1.gender=U2.gender AND U1.user_id<U2.user_id AND ABS(U1.YEAR_OF_BIRTH-U2.YEAR_OF_BIRTH)<"+yearDiff
                +" AND NOT EXISTS (SELECT USER1_ID, USER2_ID FROM " + friendsTableName+" F WHERE (U1.USER_ID=F.USER1_ID AND U2.USER_ID=F.USER2_ID)
            OR (U1.USER_ID=F.USER2_ID AND U2.USER_ID=F.USER1_ID))
            GROUP BY U1.USER_ID, U2.USER_ID
            ORDER BY U1.USER_ID, COUNT(T1.TAG_PHOTO_ID) DESC");
            ...
            while(rst.next()&&count<n){
                ...
                try(Statement stmt2=
                        oracleConnection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
                            ResultSet.CONCUR_READ_ONLY)){
                    ResultSet rst2=stmt2.executeQuery("SELECT U1.FIRST_NAME, U1.LAST_NAME, U1.YEAR_OF_BIRTH, U1.FIRST_NAME, U1.LAST_NAME, U1.YEAR_OF_BIRTH FROM "
                    ...
                    while(rst2.next()){
                        ...
                        try(Statement stmt3=
                            ...
                            ResultSet rst3=stmt3.executeQuery("SELECT P.PHOTO_ID, P.ALBUM_ID, A.ALBUM_NAME, P.PHOTO_CAPTION, P.PHOTO_LINK FROM "+userTableName+" U1, "
                                ...
                                " WHERE U1.USER_ID="+u1UserId+" AND U2.USER_ID="+u2UserId+" AND U1.user_id=T1.tag_subject_id AND U2.user_id=T2.tag_subject_id 
                                AND T1.tag_photo_id=T2.tag_photo_id AND P.PHOTO_ID=T1.TAG_PHOTO_ID AND P.ALBUM_ID=A.ALBUM_ID"
                                );
                            while(rst3.next()){
