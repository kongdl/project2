...
public class MyFakebookOracle extends FakebookOracle {
	...
	try{
            Statement stmt=oracleConnection.createStatement();
            ResultSet rst=stmt.executeQuery("With Nf As(select U1.user_id as id1,U2.user_id as id2 from "+
                userTableName+" U1,"+userTableName+" U2 where U1.user_id<U2.user_id minus select F.user1_id as id1,F.user2_id as id2 from "+
                friendsTableName+" F),fullF As (select F.user1_id as f1,F.user2_id as f2 from "
                +friendsTableName+" F union select F.user2_id as f1,F.user1_id as f2 from "+
                friendsTableName+" F) select * from (select Nf.id1, Nf.id2, count(*) from Nf, fullF F1, fullF f2 where Nf.id1=F1.f1 and Nf.id2=F2.f1 and F1.f2=F2.f2 group by Nf.id1,Nf.id2 order by count(*) desc,Nf.id1,Nf.id2) where ROWNUM <="+n);
             //int count=0;
	     while(rst.next()){
